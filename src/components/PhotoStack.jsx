...
const STATUS = {
    ...
    DELETING: 'deleting',     // Animating out
};
...
const PhotoStack = ({ photo, nextPhoto, handState, onSortLeft, onSortRight, onDelete }) => {
    ...
    // Removed scale transform: It caused a Hook Violation and is handled by variants anyway
    ...
    useEffect(() => {
        // 0. GLOBAL SAFETY: If tracking is lost, reset everything unless we are already deleting
        if (!handState.x) {
            if (status !== STATUS.DELETING && status !== STATUS.IDLE) {
        ...
        // If we haven't seen an open hand yet, checks if hand is open now
            ...
            case STATUS.PRE_DELETE:
                ...
                // -> DELETING (Timer complete)
                    ...
                    if (elapsed > FIST_HOLD_TIME) {
                        ...
                        setTimeout(onDelete, 400); // Wait for animation, then delete
                    }
                }
                break;
            ...
            case STATUS.DRAGGING:
                ...
                if (!handState.isPinching) {
                    ...
                    if (currentX < -THRESHOLD) {
                        // Sort Left
                        onSortLeft();
                        setStatus(STATUS.IDLE); // Or a 'SORTING' state, but component will unmount
                    ...
                    // Reset position visually happens via variants 'idle'
                }
                // UPDATE POSITION
                else {
    ...
    const variants = {
        ...
        [STATUS.DRAGGING]: {
            scale: 1.1, // Clear pop when grabbed
            transition: { duration: 0.2 }
        },
        [STATUS.PRE_DELETE]: {
            scale: 0.9,
            rotate: [0, -4, 4, -4, 4, 0],
            filter: ["grayscale(0%)", "grayscale(100%)", "grayscale(100%) brightness(0.8) sepia(1) saturate(500%) hue-rotate(-50deg)"], // Flash red
            transition: {
        ...
        [STATUS.DELETING]: {
            ...
            transition: {
                duration: 0.35,
                ease: [0.32, 0, 0.67, 0] // cubic-bezier for "suck" effect
            }
        }
    };

    // Dynamic Scale for Drag Feedback
    // Shrinks from 1.1 (center) to 0.6 (edges) to indicate "dropping"
    ...
    return (
        <div className="relative w-[300px] h-[450px] md:w-[400px] md:h-[600px] flex items-center justify-center">
            ...
            <motion.div
                variants={variants}
                animate={status}
                // Apply dragScale only when dragging to override the variant's static scale
